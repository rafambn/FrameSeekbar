...
import com.rafambn.framebar.api.FrameBarAPI
import com.rafambn.framebar.api.MarkersAPI
import com.rafambn.framebar.api.OnFrameBarChangeListener
import com.rafambn.framebar.api.PointerAPI
...
import com.rafambn.framebar.managers.MarkerManager
import com.rafambn.framebar.managers.PointerManager
 ...
 */
class FrameBar(context: Context, attrs: AttributeSet) : View(context, attrs),
    FrameBarAPI, MarkersAPI, PointerAPI {
    ...
    private var mOnFrameBarChangeListener: OnFrameBarChangeListener? = null

    init {
        val myAttrs = context.obtainStyledAttributes(attrs, R.styleable.FrameBar)
        mMarkerManager.createMarkers(
            myAttrs.getInt(R.styleable.FrameBar_size, 10),
            myAttrs.getInt(R.styleable.FrameBar_markersWidth, 5),
            myAttrs.getInt(R.styleable.FrameBar_markersHeight, 20),
            myAttrs.getInt(R.styleable.FrameBar_markersTopOffset, 0),
            myAttrs.getInt(R.styleable.FrameBar_markersColor, Color.GRAY),
            try {
                ContextCompat.getDrawable(
                    context,
                    myAttrs.getResourceId(R.styleable.FrameBar_markersDrawable, -1)
                )
        ...
        mPointerManager.createPointer(
            myAttrs.getInt(R.styleable.FrameBar_pointerWidth, 5),
            myAttrs.getInt(R.styleable.FrameBar_pointerHeight, 40),
            myAttrs.getInt(R.styleable.FrameBar_pointerTopOffset, 0),
            myAttrs.getInt(R.styleable.FrameBar_pointerColor, Color.YELLOW),
            try {
                ContextCompat.getDrawable(
                    context,
                    myAttrs.getResourceId(R.styleable.FrameBar_markersDrawable, -1)
        ...
        mSelectedIndex = myAttrs.getInt(R.styleable.FrameBar_startIndex, 0)
        mMovement = when (myAttrs.getInt(R.styleable.FrameBar_movement, 1)) {
            0 -> Movement.DISCRETE
        ...
        mPointerSelection =
            when (myAttrs.getInt(R.styleable.FrameBar_pointerSelection, 1)) {
        ...
        mCoercedPointer = when (myAttrs.getInt(R.styleable.FrameBar_coercePointer, 1)) {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                mOnFrameBarChangeListener?.onStartTrackingTouch(this)
                return true
            }
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                mOnFrameBarChangeListener?.onIndexChanged(this, mSelectedIndex, true)
            ...
            MotionEvent.ACTION_UP -> {
                mOnFrameBarChangeListener?.onStopTrackingTouch(this)
            }
        }
     ...
     *
     * @see OnFrameBarChangeListener
     */
    fun setOnFrameProgressBarChangeListener(listener: OnFrameBarChangeListener) {
        mOnFrameBarChangeListener = listener
    }
    ...
    override fun setIndex(index: Int) {
        ...
        mCurrentOffset = mStartOffset - mMarkerManager.findOffsetTroughIndex(mSelectedIndex)
        mOnFrameBarChangeListener?.onIndexChanged(this, mSelectedIndex, false)
        invalidate()
    }
